...
class OutputTypeConstraint(BaseModel):
    """输出类型约束"""
    name: str = Field(..., description="约束名称")
    data_type: str = Field(..., description="数据类型")
    required: bool = Field(default=True, description="是否必需")
    structure: Optional[Dict] = Field(default=None, description="数据结构定义")
    ...
    def validate_data_type(cls, v):
        ...
        valid_types = {
            ...
            'image', 'audio', 'video', 'file', 'json', 'xml', 'html'
        }
    ...
    def _check_structure(self, data: Any) -> bool:
        """检查数据结构"""
        if not self.structure or not isinstance(data, dict):
            return True
        
        try:
            for key, expected_type in self.structure.items():
                if key not in data:
                    logger.warning(f"约束 {self.name}: 缺少必需字段 {key}")
                    return False
                
                if not isinstance(data[key], expected_type):
                    logger.warning(f"约束 {self.name}: 字段 {key} 类型错误，期望 {expected_type}")
                    return False
            
            return True
            
        except Exception as e:
            logger.error(f"约束 {self.name}: 结构检查失败 {e}")
            return False
...
class OutputConstraintSet(BaseModel):
    """输出约束集合"""
    constraints: List[OutputTypeConstraint] = Field(default_factory=list, description="约束列表")
    ...
    def get_quality_metrics(self) -> List[str]:
        ...
        metrics = set()
        for constraint in self.constraints:
            metrics.update(constraint.quality_metrics)
        return list(metrics) 